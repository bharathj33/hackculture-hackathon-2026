...
import re  # StoryCritic: agent allowlist matching
...
class ZepEntityReader:
    ...
    def filter_defined_entities(
        ...
        # StoryCritic: env-gated agent allowlist. When AGENT_ALLOWLIST_REGEX is
        # set, only entities whose name matches it survive. Every downstream id
        # (config agent_id, profile user_id, OASIS graph agent_id, interview
        # agent_id) is minted as a positional index over the list returned here,
        # so shrinking the list at this single choke point keeps all id spaces
        # consistent. Unset/empty = original behavior; zero matches or a broken
        # regex = loud warning and original behavior (never an empty swarm).
        allowlist_pattern = getattr(Config, 'AGENT_ALLOWLIST_REGEX', '') or ''
        if allowlist_pattern and filtered_entities:
            try:
                allowlist_re = re.compile(allowlist_pattern, re.IGNORECASE)
            except re.error as regex_err:
                logger.warning(
                    "StoryCritic AGENT_ALLOWLIST_REGEX %r is invalid (%s) - "
                    "allowlist DISABLED for this run",
                    allowlist_pattern, regex_err)
            else:
                kept_entities = [
                    ent for ent in filtered_entities
                    if allowlist_re.search(ent.name or "")
                ]
                if not kept_entities:
                    logger.warning(
                        "StoryCritic AGENT_ALLOWLIST_REGEX %r matched 0 of %d "
                        "entities - allowlist DISABLED for this run",
                        allowlist_pattern, len(filtered_entities))
                else:
                    logger.info(
                        "StoryCritic agent allowlist: kept %d of %d entities "
                        "(pattern %r)",
                        len(kept_entities), len(filtered_entities),
                        allowlist_pattern)
                    filtered_entities = kept_entities
                    # Recompute entity types from the survivors, mirroring the
                    # label logic of the loop above.
                    entity_types_found = set()
                    for ent in kept_entities:
                        custom_labels = [l for l in ent.labels
                                         if l not in ["Entity", "Node"]]
                        if defined_entity_types:
                            matching_labels = [l for l in custom_labels
                                               if l in defined_entity_types]
                            if matching_labels:
                                entity_types_found.add(matching_labels[0])
                        elif custom_labels:
