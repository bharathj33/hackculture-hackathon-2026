...
class SimulationRunner:
    ...
    @classmethod
    def _repair_wait_mode_status(cls, simulation_id: str, state) -> None:
        """StoryCritic: the reddit runner never writes reddit/actions.jsonl and
        deliberately stays alive after its rounds (interview wait-mode), so the
        monitor never reaches its completion path and run_state stays at
        "running round 0/6" forever. env_status.json is written with status
        "alive" exactly when the simulation loop finishes and wait-mode begins —
        read-repair the status to completed while the process stays up for
        interviews. Applied to cached and file-loaded states alike."""
        if state is None or state.runner_status != RunnerStatus.RUNNING:
            return
        env_file = os.path.join(cls.RUN_STATE_DIR, simulation_id, "env_status.json")
        if not os.path.exists(env_file):
            return
        try:
            with open(env_file, 'r', encoding='utf-8') as fh:
                env_state = json.load(fh).get("status")
        except Exception:
            return
        if env_state == "alive":
            ...
            logger.info(
                "StoryCritic read-repair: %s simulation loop finished "
                "(env alive) - reporting completed; process stays up for interviews",
                simulation_id,
            )
    ...
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        if simulation_id in cls._run_states:
            ...
            cls._repair_wait_mode_status(simulation_id, state)
            return state
    ...
    def get_all_actions(
        ...
        # StoryCritic: the reddit runner never writes actions.jsonl, so file reads
        # return nothing and every consumer (timeline included) sees an empty sim
        # even though the OASIS sqlite DB is full of real actions. Rebuild from
        # the DB when the files yield nothing.
        if not actions:
            actions = cls._actions_from_oasis_db(sim_dir, platform, agent_id, round_num)
    ...
    def _actions_from_oasis_db(
        ...
        """StoryCritic: rebuild AgentAction records from reddit_simulation.db.

        trace has (user_id, created_at, action, info); rounds are not recorded,
        so consecutive actions separated by <15s share a round — env.step
        batches land within a few seconds of each other.
        """
        ...
        try:
            ...
            users = {r[0]: (r[1] or "").split("\n")[0].strip()[:60]
                     for r in conn.execute("SELECT user_id, bio FROM user")}
            rows = conn.execute(
                "SELECT user_id, created_at, action, COALESCE(info,'') FROM trace "
                "WHERE action != 'sign_up' ORDER BY created_at"
            ).fetchall()
            conn.close()
        except Exception:
            logger.exception("StoryCritic: OASIS db action fallback failed")
            return []

        out: List[AgentAction] = []
        current_round, last_ts = 0, None
        for uid, created_at, act, info in rows:
            try:
                ts = _dt.fromisoformat(created_at)
            except Exception:
                ts = None
            if ts is not None and last_ts is not None and (ts - last_ts).total_seconds() > 15:
                current_round += 1
            if ts is not None:
                last_ts = ts
            try:
                args = json.loads(info) if info else {}
            except Exception:
                args = {"raw": str(info)[:500]}
            action = AgentAction(
                round_num=current_round,
                timestamp=created_at,
                platform="reddit",
                agent_id=uid if uid is not None else -1,
                agent_name=users.get(uid) or f"agent_{uid}",
                action_type=str(act).upper(),
                action_args=args if isinstance(args, dict) else {"value": args},
            )
            if platform and action.platform != platform:
                continue
            if agent_id is not None and action.agent_id != agent_id:
                continue
            if round_num is not None and action.round_num != round_num:
                continue
            out.append(action)
        return out
