...
class TimeSimulationConfig:
    ...
    # StoryCritic: clock start hour. Starting at 00:00 meant short runs simulated only
    # off-peak hours when no agent is awake, so the timeline came out empty.
    start_hour: int = 18

    # StoryCritic: fraction of awake agents activated per round, instead of the absolute
    # agents_per_hour_* range — keeps participation steady as the panel grows.
...
- reasoning (string): 简要说明为什么这样配置"""
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        return TimeSimulationConfig(
            ...
            # Set explicitly like every other field here, so it does not read as unsupported.
            start_hour=result.get("start_hour", 18),
            participation_rate=result.get("participation_rate", 0.6),
        )
