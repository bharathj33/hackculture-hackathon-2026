...
function BeatStatsRow({ stats }: { stats: BeatEngagement }) {
  ...
  return (
    <dl className="mt-3 ml-7 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
...
function BeatRow({
  ...
  return (
    <li className={cn('px-4 py-4', failed && 'border-l-2 border-l-destructive')}>
      ...
      <blockquote className="mt-2 ml-7 border-l-2 pl-3 font-sans text-sm leading-relaxed text-muted-foreground">
        {excerpt}
      </blockquote>
      ...
      {failed && (
        <p className="mt-2 ml-7 text-sm leading-relaxed">
...
export function BeatTimeline({ className }: { className?: string }) {
  ...
  return (
    <div className={cn('flex min-h-0 flex-1 flex-col gap-4', className)}>
      <section aria-labelledby="beat-swarm-totals">
        <h2 id="beat-swarm-totals" className="sr-only">
          Swarm activity across all beats
        </h2>
        <StatPlate
          className="lg:grid-cols-[repeat(6,minmax(0,1fr))]"
          cells={[
